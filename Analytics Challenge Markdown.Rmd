...
load(file = "pde.Rdata")
# Define common route groupings
# Double moves
dm <- c("Hitch & Go", "Stick - Nod", "Out & Up", "Sluggo",
        "Corner Post", "Post Corner", "Jerk")
```
...
#Boolean evaluates whether a route concept on one side of the field has a 
#particular route from a receiver aligned in the slot or tighter
has_slot_route <- function(rte_set,concept,ofp) {
  if (contains_route(rte_set,concept)) {
   routes <- data.frame(concept,ofp) %>%
     filter(concept %in% rte_set & ofp %in% c("SWR","TE"))
   if (nrow(routes) != 0)
     return(TRUE)
  }
  return(FALSE)
...
con_namer_2 <- function(concept,ofp) {
  #First concepts that include the back to the flat
  
  # Boot
  if (concept[1] == "Comeback" &
           (concept[2] == "Whip" |
           is_flat(concept[2])))
    return("Boot")
    # Smash
  ...
  # Vert Switch
  else if (concept[1] %in% c("Post", "Seam", "Go/Fly", "Deep Cross") &
...
con_namer_3 <- function(concept,ofp) {
  
  # Start with concepts dependent on all 3 routes
  ...
  # Rub Lift
  else if ((concept[1] %in% c("Go/Fly", "Fade", "Post", "Wheel","Comeback") &
           concept[2] %in% c("Slant", "Drag", "Angle")) &
           is_flat(concept[3]))
    return("Rub Lift")
  ...
  # Spot Drag - normal
    ...
    return("Spot Drag")
    # #1 Whip replaces flat w/ minus split & Corner-Drag stacked
  else if ((concept[1] == "Whip" & is_slot(ofp[1])) &
           (contains_all_routes(c("Corner","Drag"),concept) &
            ofp[2] == ofp[3])
          )
    return("Spot Drag")
    # Bunches
            ...
            is_bunch(ofp)) &
            (has_flat(concept) | "Whip" %in% concept)
           )
    return("Spot Drag")
  ...
  # Sail Drag - normal
    ...
    return("Sail Drag")
    # #1 Whip replaces flat w/ minus split & Out-Drag stacked
  else if ((concept[1] == "Whip" & is_slot(ofp[1])) &
           (contains_all_routes(c("Out","Drag"),concept) &
            ofp[2] == ofp[3])
          )
    return("Sail Drag")
    # Bunches
  ...
  #Storm (Drag-Dig-Cross combos w/ minus split)
  else if ((concept[1] == "Drag" & is_slot(ofp[1])) &
           contains_all_routes(c("Dig", "Deep Cross"),concept[2:3])
           )
    return("Storm")
    # #1 Cross w/ Dig and Drag stacked
  else if ((concept[1] == "Deep Cross" & is_slot(ofp[1])) &
           (contains_all_routes(c("Dig", "Drag"),concept[2:3]) &
            ofp[2] == ofp[3])
           )
    return("Storm")
    # Bunches
  else if (contains_all_routes(c("Dig", "Drag","Deep Cross"),concept) &
           (is_slot(ofp[1]) & is_bunch(ofp))
           )
    return("Storm")
  ...
  # Fan
  else if ((concept[1] == "Out" | is_flat(concept[1])) &
           (con_namer_2(concept[2:3],ofp[2:3]) == "Spacer" & is_slot(ofp[1]))
          )
    return("Fan")
  
  # Cruise Lift (Inside Drive w/ slot vertical)
  else if ((concept[1] == "Drag" & is_slot(ofp[1])) &
           (contains_route(c("Dig","Curl","Over Ball"),concept) &
            contains_route(c("Go/Fly", "Fade"),concept))
           )
    return("Cruise Lift")
    # Tight bunch
  else if (("Drag" %in% concept & all(ofp == "TE")) &
           (contains_route(c("Dig","Curl","Over Ball"),concept) &
            contains_route(c("Go/Fly", "Fade"),concept))
           )
    return("Cruise Lift")
  ...
  # Out Cruise
  else if ((concept[3] %in% c("Dig", "Curl", "Over Ball") &
           contains_route(c("Drag","Over Ball"),concept)) &
           ("Out" %in% concept & is_slot(ofp[1]))
           )
    return("Out Cruise")
    # Invert #2 and #3 w/ tight stack
  else if ((concept[3] %in% c("Drag","Over Ball") &
           (concept[2] %in% c("Dig", "Curl", "Over Ball") & 
            all(ofp[2:3] == "TE"))) &
           ("Out" %in% concept & is_slot(ofp[1]))
           )
  ...
  # (Double Move) Cruise
  else if ((concept[2] %in% c("Drag","Over Ball") & is_slot(ofp[2])) &
           (concept[3] %in% c("Dig", "Curl", "Over Ball") &
            concept[1] %in% dm)
           )
    return(paste(concept[1],"Cruise"))
  # Invert #2 and #3 w/ tight stack 
  else if ((concept[3] %in% c("Drag", "Over Ball") & all(ofp[2:3] == "TE")) &
           (concept[2] %in% c("Dig", "Curl", "Over Ball") &
            concept[1] %in% dm)
           )
    return(paste(concept[1],"Cruise"))
    # Minus split #1 Drag
  else if ((concept[1] %in% c("Drag","Over Ball") & is_slot(ofp[1])) &
           (contains_route(c("Dig", "Curl", "Over Ball"),concept) &
            contains_route(dm,concept))
           )
    return(paste(intersect(concept,dm),"Cruise"))

  # Cruise (Inside Drive)
  else if ((concept[2] %in% c("Drag","Over Ball") & ofp[1] != ofp[2]) &
           concept[3] %in% c("Dig", "Curl", "Over Ball")
           )
    return("Cruise")
    # Invert #2 and #3 w/  tight stack
  ...
  else if (has_slot_route("Drag",concept,ofp)) {
    i <- max(which(concept == "Drag"))
    name <- con_namer_2(concept[-i],ofp[-i])
    return(paste(name, "Shallow"))
  }
  ...
  # (Concept)  Y Cross
    ...
    con2 <- con_namer_2(concept[1:2],ofp)
...
```{r}
#Function for naming route concepts with 4 active routes

con_namer_4 <- function(concept,ofp) {
  # #3 Drag w/ back to flat
  if ((concept[3] == "Drag" & is_slot(ofp[3])) &
      is_flat(concept[4])) {
    name <- con_namer_3(concept[c(1,2,4)], ofp[c(1,2,4)])
    return(paste(name, "Shallow"))
  }

  # #1 Drag out of slot or tight bunch w/ back to flat
  else if ((concept[1] == "Drag" & is_bunch(ofp[1:3])) &
      is_flat(concept[4])) {
    name <- con_namer_3(concept[2:4], ofp[2:4])
...
half_field_namer <- function(lr_string) {
  ...
  #Function
  num <- length(concept)
  #Add the back to the flat to the concept for naming purposes
  if (! contains_route("",back_routes)) {
    concept <- c(concept,"Flat")
    ofps <- c(ofps,"B")
    num <- num + 1
  }
  if (num == 1) {
  ...
  else if (num == 2) {con_namer_2(concept,ofps)}
  #Sort to 3-man concept namer
  else if (num == 3) {con_namer_3(concept,ofps)}
...
split_routes2 <- filter(split_routes, LiveRoutes == 2)
split_routes3 <- filter(split_routes, LiveRoutes == 3)
```

```{r}

split_routes2$ConName <- sapply(split_routes2$StringIn,half_field_namer)

split_summary2 <- split_routes2 %>%
  group_by(ConName) %>%
  mutate(ConNum = sum(n)) %>%
  arrange(desc(ConNum),ConName, desc(n))

ss2 <- filter(split_summary2, ConName != "Unnamed")

unnamed2 <- filter(split_summary2, ConName == "Unnamed")
...
unnamed3 <- filter(split_summary3, ConName == "Unnamed")
```
